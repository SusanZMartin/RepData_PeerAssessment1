...
---
title: "Peer Assessment Project Reproducible Research"
...
The mean for 'nighttime' intervals (midnight to 6:00 am) was `r nighttime` steps per interval and for 'daytime' intervals (6:05 am to 11:55 pm) it was `r daytime` steps per interval. These numbers were rounded to 3 and 49 for imputation.
...
```{r imputationprocess, cache = TRUE}
...
The plot above shows the total number of steps per day for each of the 61 days within the monitoring period where the missing values have been replaced with imputed values. The mean number of steps per day is now `r impmeanspd` (versus `r meanspd` in the previous histogram) and is represented by the red dashed line. The median number of steps per day is now `r impmedianspd` (versus `r medianspd` in the previous histogram) and is represented by the yellow dashed line. Note that because of the closeness of the mean and median values it is difficult to see the lines indicating them on the histogram but they are both there.
...
Analysis shows that there was a 19% higher mean number of steps per day on the weekends than on the weekdays (`r totalspdwkend` versus `r totalspdwkday` respectively).